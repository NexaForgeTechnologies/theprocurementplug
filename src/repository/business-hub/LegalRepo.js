    ...
    try {
      ...
      return rows.map((row) => ({
        ...
        jurisdictional_coverage: row.jurisdictional_coverage
          ? row.jurisdictional_coverage.split(",").map((s) => s.trim())
          : [],
    ...
    try {
      const [rows] = await db.query(
        `SELECT 
          ...
          LEFT JOIN industries i ON lc.industry_id = i.id
          LEFT JOIN regions r ON lc.region_id = r.id
          WHERE lc.deleted_at IS NULL AND lc.legal_compliance_type_id = ?;
        `,
        [id]
      );
      ...
      return {
        ...
        overview: row.overview,
        email: row.email,
        // ✅ Split or parse coverage correctly
        jurisdictional_coverage: row.jurisdictional_coverage
          ? row.jurisdictional_coverage.split(",").map((s) => s.trim())
          : [],
        company_logo: row.company_logo || [],
      ...
      console.error("Error fetching legal_compliance by ID:", error);
      throw new Error("Unable to fetch legal_compliance by ID from the database");
    }
  }
}
