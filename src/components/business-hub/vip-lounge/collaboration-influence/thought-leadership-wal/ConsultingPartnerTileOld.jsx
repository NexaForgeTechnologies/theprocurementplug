...
//     <Link
//       href={url}
//       className="w-full rounded-[6px] border border-[#DBBB89] hover:border-[#85009D] bg-[#FFFBF5] hover:bg-[#85009D] p-5 flex flex-col-reverse lg:flex-row text-center lg:text-start justify-center lg:justify-between gap-6  text-[#85009D] hover:text-white transition-all duration-200 ease-in-out group"
//     >
...
//             width={204}
//             height={204}
//             className="w-[204px] h-[204px] lg:w-full mb-2 border border-[#B08D57] rounded-full"
//           />
//           {(vom || pick) && (
      ...
      <div className={`${isCol2 ? 'gap-[14px]' : 'gap-4'} flex flex-1 flex-col items-center lg:items-start`}>
        ...
        <span className="flex flex-wrap">
          {smallimg && Array.isArray(smallimg) && smallimg.length > 0 ? (
            ...
            btntext && (
              <span className={`flex self-start items-center cursor-pointer bg-[#B08D57] hover:bg-[#85009D] text-white px-4 py-2 rounded-[6px]`}>
                {btntext}
                <div className="ml-1 w-2 h-2 border-t-2 border-r-2 border-white transform rotate-45"></div>
              </span>
            )
