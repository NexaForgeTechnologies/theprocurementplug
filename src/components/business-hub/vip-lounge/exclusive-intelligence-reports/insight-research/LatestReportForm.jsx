...
export default function LatestReportForm({ isOpen, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch("/api/business-hub/vip-lounge/exclusive-intelligence-reports/industry-insights/download-latest-report", {
                method: "POST",
                headers: {
            ...
            triggerDownload("/files/CONCIERGE_TASKS.pdf", "CONCIERGE_TASKS.pdf"); // ✅ customize the path & filename
    ...
    const triggerDownload = (fileUrl, fileName = "Latest-Report.pdf") => {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input
                            ...
                            required
                            className="w-full p-4 border-1 border-[#85009D] rounded-[2px] bg-white focus:outline-none focus:ring-1 focus:ring-[#85009D]"
                        />
                        <input
                            type="email"
                            name="email"
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className={`flex items-center justify-center md:justify-start bg-[#b08d57] text-white px-4 py-2 rounded-[6px] w-full md:w-auto ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
