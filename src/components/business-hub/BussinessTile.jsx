...
function ConsultingPartnerTile({ heading, para, btntext, url = "", bgImage = "", isLast = false }) {
    return (
        <Link
            href={url}
        ...
        >
            <h2 className={` ${isLast ? "text-white" : "text-[#85009D] group-hover:text-white"} text-xl md:text-2xl font-semibold`}>{heading}</h2>
            {para && (
                <p className={` ${isLast ? "text-white" : "text-[#1b1b1b]"} ${isLast ? "max-w-full sm:max-w-[400px]" : "max-w-full sm:max-w-[250px]"} text-[#1B1B1B] text-base group-hover:text-[#ffff]`}>
                    {para}
                </p>
            )}
            {btntext && (
