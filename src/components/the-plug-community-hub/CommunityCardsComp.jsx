...
export default function CommunityCardsComp({src, title, para, btnName}) {
    return (
        <div className="border border-[#DBBB89] bg-[#FFFBF5] rounded p-4 flex flex-col justify-start gap-3">
            ...
            <h1 className="text-[#85009D] font-semibold text-base lg:text-xl mt-1 break-words">{title}</h1>
            <p className="text-black text-sm md:text-base break-words -mt-1">
                {para}</p>
            <div className="self-start mt-auto">
                <ArrowButtonCom text={btnName} bgColor={"bg-[#B08D57]"} textColor={"text-white self-start mt-auto"} hoverBgColor={"hover:bg-[white]"} hoverTextColor={"hover:text-black "} borderColor={"border-[#B08D57]"} />
            </div>
        </div>
    )
}
