  ...
  return (
    <a href={href} target={targetLink}>
      <button
        ...
        className={`
          ${bgColor} ${hoverBgColor} ${textColor} ${hoverTextColor} ${className}
          font-normal text-xs sm:text-sm lg:text-base rounded-full flex items-center duration-300 group
          ${loading ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}
        `}
      >
        {/* Text section */}
        <span className="rounded-full bg-[#0F0F0F] text-[#B08D57] mx-1 my-1 px-2.5 sm:px-4 py-2">
          {loading ? (
