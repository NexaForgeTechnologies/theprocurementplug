...
export async function POST(req) {
  try {
    ...
    (async () => {
      let template = status === "approved"
        ? "host-round-table-approved.html"
        : "host-round-table-declined.html";

      let subject = status === "approved"
        ? "Application Approved"
        : "Application Declined";

      let html = await emailParser(template, {
        name: check[0].name,
        year: new Date().getFullYear()
      });

      sendEmail(check[0].email, subject, html).catch(console.error);
    })();
