...
import { useEvent } from "@/hooks/GetEvent";
...
function ResourceLibraryCTR() {
  ...
  const { data: events, eventLoading, error, refetch } = useEvent();
  ...
  const filteredEvents = events.filter((event) => {
    const matchesType = selectedType
      ? event.event_name.toLowerCase() === selectedType.toLowerCase()
      : true;
    ...
    const matchesHeading = searchText
      ? event.event_heading.toLowerCase().includes(searchText.toLowerCase())
      : true;
    ...
    <>
      <div>
        ...
        <p className="max-w-[930px] md:text-xl text-[#1B1B1B] my-4">
          Whether you're looking to deepen your knowledge, build practical
          skills, or expand your professional network, our diverse event lineup
          has something for everyone. Join us for seminars that dive into expert
          insights, webinars you can attend from anywhere, workshops designed
          for hands-on learning, and networking opportunities that connect you
          with like-minded professionals and industry leaders.
        </p>
      ...
      <div>
        ...
        <p className="max-w-[930px] md:text-xl text-[#1B1B1B] my-4">
          The Procurement Plug Flagship Event Series brings together industry
          leaders to explore trends, share insights, and shape the future of
          procurement.
        </p>
      ...
      <div>
        ...
        <p className="max-w-[930px] md:text-xl text-[#1B1B1B] my-4">
          We have a wide range of upcoming events planned, including seminars,
          webinars, workshops, and networking opportunities. Stay tuned for more
          details!
        </p>
        ...
        <div className="mt-10 grid gap-6 md:gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
          <div className="w-full bg-[#85009D] border border-[#85009D] rounded-[8px] p-4 shadow-md hover:shadow-lg transition duration-300">
            <Image
              width={240}
              height={127}
              src="/images/events/global.png"
              alt="Special Event Banner"
              className="mb-4 mx-auto"
            />
            <p className="text-[12px] text-[#D09B48] font-medium mb-2 text-center md:text-left pl-[0px] md:pl-[10px]">
              Join our BI-Monthly Webinar Series
            </p>
            <h3 className="text-[24px] md:text-[32px] font-semibold text-[#D09B48] leading-none text-center md:text-left pl-[0px] md:pl-[10px]">
              Procurement Beyond Borders
            </h3>
          </div>
          {events.map((event) => (
            <EventComp key={event.id} data={event} />
