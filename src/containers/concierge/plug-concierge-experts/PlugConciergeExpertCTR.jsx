        ...
        <>
            <div>
                ...
                <div className='bg-[#85009D] rounded-[6px] p-5 flex flex-col md:flex-row items-start md:items-center justify-between'>
                    <div>
                        <h3 className="font-semibold text-xl text-[#FFFFFF]">Ready to become a Plug Concierge Expert</h3>
            ...
            <div>
                <h3 className="font-semibold text-lg md:text-xl text-[#1B1B1B] mb-4">Welcome to Plug Concierge Experts</h3>
                <p className="font-semibold text-lg md:text-xl text-[#1B1B1B] mb-4">What We Do</p>
                <p className="max-w-[986px] md:text-lg text-[#1B1B1B] mb-4">Plug Concierge connects exceptional procurement experts with businesses who need on-demand, tailored support.</p>
                <p className="max-w-[986px] md:text-lg text-[#1B1B1B]">From rapid sourcing to enterprise-wide transformation, we ensure every task is matched to the right expert level and always reviewed for strategic quality.</p>
            </div>
            ...
            <div>
                ...
                {howItWorksData.map((item, idx) => (
                    <div key={idx} className="flex gap-2 md:gap-4 items-start mb-4 md:mb-8">
                        ...
                        <div>
                            <h3 className="font-medium text-base md:text-lg text-[#1B1B1B] mb-2">{item.title}</h3>
                            <p className="max-w-[872px] text-[16px] md:text-base text-[#1B1B1B]">{item.text}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-[#FFFBF5] text-[#1B1B1B] overflow-x-auto">
                <table className="w-full border-collapse bg-[#FFFBF5] text-left border-2 border-[#DBBB89] rounded-[6px] table-fixed min-w-[600px] md:min-w-0">
                    <thead>
                        <tr className="bg-[#FFFBF5] text-sm md:text-base text-[#85009D] font-medium">
                            <th className="p-4 border-2 border-[#DBBB89]/50 w-1/4">Task Level</th>
                            <th className="p-4 border-2 border-[#DBBB89]/50 w-1/4">Handled by</th>
                            <th className="p-4 border-2 border-[#DBBB89]/50 w-1/4">QA by</th>
                            <th className="p-4 border-2 border-[#DBBB89]/50 w-1/4">Examples</th>
                        </tr>
                    </thead>
                    <tbody>
                        {taskTableData.map((row, idx) => (
                            <tr key={idx} className="bg-[#FFFBF5] text-sm font-medium">
                                <td className={`p-4 border-2 border-[#DBBB89]/50 ${row.levelColor} w-1/4`}>{row.level}</td>
                                <td className="p-4 border-2 border-[#DBBB89]/50 w-1/4">{row.handledBy}</td>
                                <td className="p-4 border-2 border-[#DBBB89]/50 w-1/4">{row.qaBy}</td>
                                <td className="p-4 border-2 border-[#DBBB89]/50 w-1/4">{row.examples}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div>
                <h3 className="font-semibold text-lg md:text-xl text-[#1B1B1B] mb-4">The Process in Detail</h3>
                {processSteps.map((step, idx) => (
                    <div key={idx} className="flex gap-2 md:gap-4 items-start mb-4 md:mb-6">
                        <div className="w-[10px] md:w-[15px] h-[10px] md:h-[15px] bg-[#B08D57] rounded-full mt-[7px] shrink-0"></div>
                        <div>
                            <h3 className="font-semibold text-base text-[#1B1B1B] mb-1">{step.title}</h3>
                            <p className="md:text-base text-[#1B1B1B]">{step.text}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-[#FFFBF5] py-6 md:py-10"
                ...
                <div className='w-full md:flex-1'>
                    <h3 className="md:max-w-[348px] text-lg md:text-2xl text-[#85009D] leading-none">Why Join</h3>
                    <h3 className='font-semibold md:max-w-[348px] text-lg md:text-2xl text-[#85009D]'>Plug Concierge?</h3>
                    <p className="md:max-w-[417px] text-base text-[#1B1B1B] mt-4">Become a Plug Concierge Expert and Join our Network of Professional</p>
                </div>
                <div className='w-full md:flex-1'>
                    {benefits.map((benefit, idx) => (
                        <div key={idx} className="flex gap-2 md:gap-4 items-start mb-4">
                            ...
                            <p className="md:text-base text-[#1B1B1B]">{benefit}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="rounded-[6px] border border-[#DBBB89] bg-[#FFFBF5] p-5 w-full lg:self-start hover:text-[#ffff]">
                <h3 className="font-semibold text-lg md:text-xl text-[#85009D]">Advance Even Further: Become a Plug Strategic QA Partner</h3>
                ...
                {qaPartnerPerks.map((perk, idx) => (
                    <div key={idx} className="flex gap-2 md:gap-4 items-start mb-4">
                        <div className="w-[10px] md:w-[15px] h-[10px] md:h-[15px] bg-[#B08D57] rounded-full mt-[7px] shrink-0"></div>
                        <p className="md:text-base text-[#1B1B1B]">{perk}</p>
                    </div>
                ))}
                {/* <Link
